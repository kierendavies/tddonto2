...
public class ManchesterOWLSyntaxParserFixed implements ManchesterOWLSyntaxParser {
    @Nonnull
     ...
     */
    @Inject
    public ManchesterOWLSyntaxParserFixed(
    ...
    private OWLSameIndividualAxiom parseSameIndividualFixed() {
        String section = consumeToken();
        if (!SAME_INDIVIDUAL.matches(section)) {
            throw new ExceptionBuilder().withKeyword(SAME_INDIVIDUAL).build();
        }
    ...
    private OWLDifferentIndividualsAxiom parseDifferentIndividualsFixed() {
        String section = consumeToken();
        if (!DIFFERENT_INDIVIDUALS.matches(section)) {
            throw new ExceptionBuilder().withKeyword(DIFFERENT_INDIVIDUALS)
                    .build();
    ...
    public OWLAxiom parseAxiom() {
        ...
        } else if (SAME_INDIVIDUAL.matches(token)) {
            return parseSameIndividualFixed();
        } else if (DIFFERENT_INDIVIDUALS.matches(token)) {
            return parseDifferentIndividualsFixed();
        }
        throw new ExceptionBuilder().withClass().withObject().withData()
    ...
    private OWLAxiom parseAxiomWithIndividualStart() {
        ...
        } else if (SAME_AS.matches(kw)) {
            Set<OWLIndividual> individuals = parseIndividualList();
            individuals.add(ind);
            return dataFactory.getOWLSameIndividualAxiom(individuals);
        } else if (DIFFERENT_FROM.matches(kw)) {
            Set<OWLIndividual> individuals = parseIndividualList();
            individuals.add(ind);
            return dataFactory.getOWLDifferentIndividualsAxiom(individuals);
        }
        throw new ExceptionBuilder().withKeyword(TYPE, SAME_AS, DIFFERENT_FROM).build();
    }
